...
- `fkst.observe([opts]) -> table` is the in-process Lua primitive for the existing `fkst-framework observe --json` durable delivery snapshot; `opts.limit` defaults to 500 and must be within 1..10000. It resolves the same `DurableLayout` from `FKST_DURABLE_ROOT`, reads the live observe socket when available, otherwise opens `<DURABLE>/delivery.redb` read-only through the existing offline path, and returns the same `DeliveryObserveSnapshot` as a Lua table. It does not introduce new observe semantics and does not shell out to `fkst-framework`.
