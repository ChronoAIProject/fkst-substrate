...
- runtime layout 是 host-local scratch；事件队列、raise buffer、locks、logs 不构成实体业务真相，也不跨机同步。`marks` 只承载 `once` success marker，`cache` 只承载 `cache_get` / `cache_set` / `cache_expire` 的 best-effort scratch KV 与 opt-in `exec_sync` read coalescing success entries，不承载业务 schema、accepted-state 或 rollback state。可靠在途 delivery 状态落在 `FKST_DURABLE_ROOT` 的 redb delivery store；它只承载 delivery lease / retry / DLQ 账本，不承载实体业务真相。visible-intent 状态也落在 `FKST_DURABLE_ROOT`；它只承载 non-idempotent effect 的 intent/result ledger、`effect_key` idempotency barrier 和 recovery fencing，不承载 package business state、accepted-state 或 rollback state。恢复由 cron / file_watch scanner 从 durable 事实 re-derive 并重新 enqueue；`<RT>` 被清空时 cache miss 返回 nil，不影响 redb delivery store 或 visible-intent ledger。host main repo HEAD/index 不承载 `<RT>/worktrees`、`<RT>/codex-permits`、`<RT>/locks`、`<RT>/logs`、`<RT>/marks`、`<RT>/cache` runtime layout pathspec。
...
- 固定 Lua SDK surface 锚点是 `fixed-lua-sdk-surface`；允许 surface 是 `pipeline`、`source`、`raise`、`declare_intent`、`wait_until_intent_visible`、`perform_or_recover_effect`、`write_result_marker`、`wait_until_result_visible`、`derive_next_transition_from_visible_result`、`spawn_codex_sync`、`spawn_codex`、`fkst.codex_runs`、`exec_sync`、`await_all`、`with_lock`、`once`、`cache_set`、`cache_get`、`cache_expire`、`graph_json`、`t`、`git_log_count`、`git_log_grep`、`count_worktrees`、`list_orphan_worktrees`、`setup_worktree`、`file`、`json.decode`、`log.info`、`log.warn`、`log.error`、`now`。
- `declare_intent(edge, generation, effect_kind, effect_key)` writes a durable current-generation intent under `FKST_DURABLE_ROOT`; missing `FKST_DURABLE_ROOT` fails closed. Re-declaring the same tuple is idempotent; rebinding an `effect_key` to another intent fails closed. `wait_until_intent_visible(intent_id)` only returns after the intent is readable from the durable ledger. `perform_or_recover_effect(intent_id, effect_key, recover_fn, perform_fn)` requires a visible matching intent, takes an engine-owned lock for that intent, checks for an existing result, calls `recover_fn(effect_key)` before `perform_fn(effect_key)`, and writes a result marker for the recovered or performed result. `write_result_marker(intent_id, result)` is idempotent: the first visible result wins. `wait_until_result_visible(result_id)` reads the durable result marker, and `derive_next_transition_from_visible_result(result_id)` returns the visible result payload. This primitive is a transactional-outbox / durable-workflow barrier for non-idempotent effects; it does not provide a package workflow scheduler, external API adapter, business-state store, accepted-state, rollback state, or exactly-once external side effects.
