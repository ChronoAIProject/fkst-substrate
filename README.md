...
production Lua SDK 包含 visible-intent barrier：`declare_intent(edge, generation, effect_kind, effect_key)`、`wait_until_intent_visible(intent_id)`、`perform_or_recover_effect(intent_id, effect_key, recover_fn, perform_fn)`、`write_result_marker(intent_id, result)`、`wait_until_result_visible(result_id)` 和 `derive_next_transition_from_visible_result(result_id)`。它是 non-idempotent effect 的 transactional-outbox / durable-workflow primitive：intent 与 result ledger 落在 `FKST_DURABLE_ROOT`，缺失时 fail closed；同一 tuple 重复声明幂等，`effect_key` 不能重绑定；执行前必须读到 visible intent；redrive 先调用 `recover_fn(effect_key)`，只有返回 nil 才调用 `perform_fn(effect_key)`；result marker first-writer-wins。这个 primitive 不提供 package workflow scheduler、外部 API adapter、业务状态库、accepted-state、rollback state 或 exactly-once external side effect。
