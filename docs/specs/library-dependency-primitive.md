...
Per-unit `fkst.toml`: `kind = "package"|"library"`, `name`, `[code] root`, `[lib_deps]` (declared libraries), `[event_deps]` (canonical composed package event dependencies: sibling packages referenced via consumes/produces/fanout/raise for composed conformance). Library adds `[library] name/stable_id/version`, `[visibility] allow=[...]`. A package workspace's `fkst.workspace.toml` discovers units + registries; its `fkst.lock` pins them (workspace-internal = "workspace"). `fkst-substrate` owns no Lua units and carries neither file; its deployed package composition comes from the operator's deployment declaration.

Explicit `--package-root` inputs outside `--project-root` are platform roots supplied by the operator, not project units discovered from the host workspace. They use their own discovered `fkst.workspace.toml` catalog for `lib_deps` / `event_deps` and unit-scoped `require`; the host workspace does not need to own those external paths. Package roots inside `--project-root` remain project units and must be owned by the host workspace manifest.

## Recorded implementation phases (2026-06-22; retained as design history)
- **A. Manifest + catalog (legacy-safe)**: The phase plan called for `Cargo.toml` +toml; NEW `manifest.rs` (~500: parse fkst.toml/workspace/lock, types, UnitGraph/catalog, discover by walking up to fkst.workspace.toml, **legacy mode = no manifests → then-current symlink behavior**); `path_resolver.rs` (~180: optional UnitGraph in PackageRoots, manifest-aware RequireScope). The plan described no behavior change at that point (legacy fallback).
...
- **C. `fkst-framework deps` validator**: NEW `deps_cli.rs` (~450: render+validate DAG acyclic, declared lib exists, visibility, exports exist, no ambiguous, no orphan, declared lib_deps == actual require literals via a small Rust scanner = G-STD-DEP equivalent, report event_deps edges from the manifest).

## Packages side (fkst-packages)
All units get `fkst.toml`; `std` declared as a library (kind=library, all-public, visibility=public); `event_deps` is the canonical composed package dependency declaration; `lib_deps` comes from G-STD-DEP-derived actual usage. **Per-package `std` symlink REMOVED** (replaced by manifest+resolver). Package-repo scripts query composed dependencies through `fkst-framework manifest composed-deps --manifest <path>` instead of reading a duplicate deps file. Restructure `std` to the library layout (all-public).
