...
Observe schema version `1` additively exposes the authority as `terminal_suppressions`. `include = { "errors" }` selects both `dead_letters` and `terminal_suppressions`. Suppression facts have their own bounded completeness fields, `limits.max_terminal_suppressions` and `truncated.terminal_suppressions`, because their lifetime and cardinality are independent of retained DLQ diagnostics. Older live-socket snapshots without these additive fields deserialize as empty suppression facts with zero/false bounds.
