...
Failed entries additionally contain `failure_kind` and `error`. `failure_kind = "assertion_failure"` means an engine-provided `fkst.test` assertion structurally marked the error after the test function was invoked. Every other test-function error and every load or eval failure uses `failure_kind = "test_error"`; load or eval entries use `name = "<load>"`. Pass entries omit both fields. This taxonomy records where the failure originated and does not decide retryability. It does not change `status`, `error`, summary counts, test continuation, or exit codes. Report entries come from test files and `test_*` keys enumerated by Rust. Each entry is identified by the `owner_namespace`, `file`, and `name` tuple and has no delimiter-sensitive concatenated `id`. Lua `print` cannot inject report entries. The stdout `PASS`, `FAIL`, and summary lines remain a legacy human and compatibility surface, not the authoritative machine channel.
...
`fkst.test.mock_command(pattern, result)`、`fkst.test.mock_observe(snapshot)`、`fkst.test.with_command_cassette(opts, fn)` 与 `fkst.test.command_calls()` 只在 test mode 注册。mock runner 劫持 `exec_sync`、codex SDK 与 git SDK 的外部命令调用；渲染命令行按前缀或子串匹配，mock 按注册顺序一次性消费，未 mock 且无 active cassette 的外部命令 fail closed 且不启动真实进程。`mock_observe(snapshot)` makes `fkst.observe()` use that deterministic raw snapshot in the current test and in fresh Lua states spawned by `run_department` / test-runtime helpers; `fkst.observe(opts)` still applies `since`, `limit`, and `include` to the mock. Unmocked test-mode `fkst.observe()` fails closed. `command_calls()` 返回每次调用的 rendered、program、args、stdin、cwd、env、stdout、stderr 与 exit_code。`run_department` 创建的 fresh Lua state 与测试文件共享同一个 mock/cassette/observe state；每个 test function 开始前清空 mocks、active cassette、calls 与 mock observe snapshot。`setup_worktree` 在 test mode 也经过同一 git mock runner，但 mock 不模拟 worktree filesystem 副作用。
...
consumer 的可靠路径由定时 tick 和 Fanout 唤醒触发，调用 delivery store `lease` 取 due 或过期 lease 的记录，构造标准 `Event{queue,payload,ts}` 后 spawn framework。exit 0 且 RAISED 批次中所有未被终态 tombstone 抑制的 publish 成功才 `ack`；终态 tombstone 抑制项继续该批次且不阻止 `ack`，spawn error、stall、非零退出或其余 RAISED publish 失败都调用 `retry`。retry 达到 `max_attempts` 时 delivery 移入 redb dead 表，并 best-effort 经 Router publish `dead_letter` 通知；当前 delivery 自身来自 `dead_letter` 时抑制再次发送 `dead_letter`，避免自环。`Fanout` 在可靠路径只承担进程内唤醒，不再承载可靠事实。

Stable raised `dedup_key` values use subscriber-scoped keyed-workqueue coalescing. A duplicate for a ready or expired-lease record remains collapsed. A duplicate received while the delivery lease is active atomically sets one durable `pending_dirty` bit, and further duplicates remain collapsed. A successful `ack` atomically converts that dirty record into one clean ready follow-up instead of deleting it. Retry and lease-expiry transitions retain an already-set bit; a terminal DLQ transition retains its tombstone, records that the stable key remains reusable across index rebuilds, and atomically promotes the one clean follow-up. Re-enqueue against an exact terminal tombstone returns `TerminallySuppressed`; the router's `publish` compatibility entry point reports `delivery terminally suppressed`, while `publish_outcome` returns `PublishOutcome::TerminallySuppressed` and Department RAISED batch publishing skips that item before continuing the batch. For the suppressed delivery, the router journals `event=delivery_suppressed reason=terminal_tombstone` and does not journal `reason=durable_enqueue` or wake that subscriber. A different `dedup_key` derives a fresh identity and remains admissible. Non-keyed delivery behavior is unchanged.

The bounded exact-identity suppression table is both the enqueue authority and the source of structured suppression facts. Each fact contains `delivery_id` and retains `queue`, `dept`, `source`, `observed_at_ms`, `terminal_at_ms`, and `dedup_key` when available. It remains queryable for exactly as long as the identity remains suppressed; terminal DLQ detail may compact after its independent 24-hour diagnostic retention without silencing that fact. Schema migration enriches retained rows from DLQ detail and recovers the queue, Department, and stable key from compacted v3 raised delivery identities. This authority does not change the RAISED batch continuation contract.
...
| `fkst.observe([opts])` | `sdk_observe.rs`, generic durable delivery snapshot with opt-in DLQ keyset continuation and time-window filtering |
...
`fkst.observe({ limit = n, include = { "errors", "entities" }, page = { section = "dead_letters", after? = cursor } })` opts into DLQ keyset traversal. `after` is omitted for the first page. The durable store maintains `dead_by_time` in `(dead_at_ms, delivery_id)` order, and both the live-owner socket path and offline path select at most `limit + 1` indexed rows in one read transaction. A lookahead row produces the versioned, section-bound opaque `page.next`; the final page omits `next`. Malformed cursors fail closed with `observe dead-letter cursor invalid`. Each page observes the current mutable retained DLQ; the cursor prevents duplicate or skipped equal-timestamp rows in that ordering but does not freeze mutations across calls.
`fkst.observe({ limit = n, include = { "errors", "entities" }, dead_letter_window = { start_ms = a, end_ms = b } })` applies the half-open predicate `a <= dead_at_ms < b` in the same `dead_by_time` index before the display limit. A windowed result carries `dead_letters_complete`, which reports whether all currently retained rows matching that window fit within the display limit; entries outside the window cannot set the flag. This is a mutable current-DLQ projection, not an occurrence history, so redrive and retention can remove rows between calls. The window cannot be combined with a DLQ page cursor; delivery-ID `since` remains ordered independently and is applied after the bounded window read.

Observe schema version `1` additively exposes the authority as `terminal_suppressions`. `include = { "errors" }` selects both `dead_letters` and `terminal_suppressions`. Suppression facts have their own bounded completeness fields, `limits.max_terminal_suppressions` and `truncated.terminal_suppressions`, because their lifetime and cardinality are independent of retained DLQ diagnostics. `page = { section = "terminal_suppressions", after? = cursor }` traverses that authority in durable slot order with the same `limit + 1` lookahead contract. The opaque cursor carries the last delivery identity; the store resolves it back to its permanent authority slot and resumes exclusively after that slot. Malformed or unresolvable cursors fail closed with `observe terminal-suppression cursor invalid`. Older live-socket snapshots without these additive fields deserialize as empty suppression facts with zero/false bounds.
