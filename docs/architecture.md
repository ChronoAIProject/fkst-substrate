...
- `spawn_codex_sync` 带 `worktree` 时会把 prompt、stdout、stderr、trusted effect receipt log、effect marker、result marker 与 completion status 落到 writable runtime-scratch dir `<runtime-log-root>/codex-adoption/<key>/`，`key` 由可选 `dedup_key`、`worktree` 与 prompt/context identity 派生。framework 在持有该 key 的 `run.lock` 时先写入并回读可见的 `intent` status，只有 intent visible 后才允许 detached `fkst-framework __codex-worker` claim 为 `running` 并执行真实 `codex exec`；worker 自身也必须在同一 lock 下 claim visible intent，避免 parent 在 spawn 后、running 写入前退出造成 duplicate effect。`codex exec` 返回后先向 adoption-local trusted receipt log 追加 `CODEX_EFFECT:` keyed receipt，再发布 adoption-local keyed effect marker 和 result marker；redrive 可由 result marker、effect marker 或 trusted `effect_key` receipt 恢复 completed status 而不重复 effect。`log_path` remains a human/debug log that mixes status records with untrusted stdout/stderr output. It is not a recovery receipt source. 同一 work unit 的重投递先检查该 handoff：completed 则读取结果，running 则等待同一 worker/codex 完成，stale intent 或 dead running owner 则 redrive and claim before effect，不再次 storm-spawn `codex`；不同 worktree 即使使用相同 `dedup_key`、prompt 和 context 也不互相 adopt，同一 worktree 下不同 `dedup_key` 或 prompt/context 也不互相 adopt。该目录属于 RuntimeLayout scratch 之外的 process-trace scratch，用于 supervisor generation 间 adoption；它不写入 worktree，不承载 package business schema、accepted-state 或 rollback state。无 `worktree` 的 `spawn_codex_sync` 保持 pipe/wait 语义。
