...
`fkst.observe({ limit = n, include = { "errors", "entities" }, page = { section = "dead_letters", after? = cursor } })` opts into DLQ keyset traversal. `after` is omitted for the first page. The durable store maintains `delivery_by_observe_order` in `(queue, dept, not_before_ms, delivery_id)` order and `dead_by_time` in `(dead_at_ms, delivery_id)` order. Requested live and DLQ listings select at most `limit + 1` indexed records in one read transaction on both the live-owner socket and offline paths. The lookahead preserves the existing `truncated` flags and produces the versioned, section-bound opaque `page.next` for DLQ page mode; the final page omits `next`. Malformed cursors fail closed with `observe dead-letter cursor invalid`. Queue aggregates remain exact in a separate scan of live records, independent of listing limits. SDK `include` values become a storage projection before either transport path reads the store: `queues` selects exact aggregates, `events` selects live records, `errors` selects DLQ records, and `entities` selects both bounded listings for exact truncation metadata. Omitting `include` retains the full projection and byte-identical request/result serialization. Each page observes the current mutable retained DLQ; the cursor prevents duplicate or skipped equal-timestamp rows in that ordering but does not freeze mutations across calls.
