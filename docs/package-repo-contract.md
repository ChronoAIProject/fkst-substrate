...
`codex_status()` is a read-only observability query for engine codex run records. It returns running and recent entries with `role`, `started_at`, `status` (`running`, `done`, or `failed`), bounded `output_tail`, and optional `exit_code`; it does not return runtime paths or unbounded stdout/stderr.
