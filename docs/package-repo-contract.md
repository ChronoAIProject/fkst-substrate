...
`spawn_codex_sync(opts)` and `spawn_codex(opts)` accept an optional `opts.sandbox` from the closed set `"read-only"`, `"workspace-write"`, and `"danger-full-access"`. Unsupported values fail closed with `codex-sandbox-invalid`. Omitting the field preserves the byte-identical `codex exec --dangerously-bypass-approvals-and-sandbox` default so implement and fix calls retain write and network access. An explicit value emits `codex exec --sandbox <mode>` without the bypass flag. `"read-only"` denies model-generated filesystem writes and network access; a prompt that asks Codex not to write or use the network is not an equivalent boundary.
...
`fkst.observe([opts]) -> table` is the in-process form of `fkst-framework observe --json`. It returns generic durable delivery observe facts: source metadata, limits, truncation flags, queue depth state, queue `subscriber_status` (`"current"`, `"absent"`, or `"unknown"`), live delivery entries, and DLQ entries, with payloads summarized by schema, dedup key, byte count, and digest rather than full bodies. `opts` is optional and may only narrow generic observe data with `limit`, `include = {"queues","errors","events","entities"}`, `since = <delivery_id>`, and `page = { section = "dead_letters", after = <opaque cursor> }`; omit `after` for the first page, and do not combine `page` with `since`. Page mode traverses retained DLQ rows in `(dead_at_ms, delivery_id)` order and adds `page.section` plus `page.next` only when another bounded page exists. Live and DLQ listings each read at most `limit + 1` ordered records, while exact queue aggregates use an independent live-record scan. `include` is pushed into storage before either the live-owner socket or offline path reads: callers that need queue health should request `queues`, callers that need live records should request `events`, and callers that need DLQ records should request `errors`; `entities` requests both bounded listings because it includes their truncation metadata. Omitting `include` requests the full projection. The opaque cursor is versioned and section-bound; malformed cursors fail closed. Each call reads the current mutable store and does not provide cross-request snapshot isolation. Calls without `page` retain their byte-identical result shape. The surface must not encode business concepts such as idle, board, audit, skip, or workflow phases. Production reads `FKST_DURABLE_ROOT` and uses the same live owner-process socket / offline database projection as the CLI. Package-side helpers may interpret the generic facts, but the engine does not. Offline observations report `subscriber_status = "unknown"` because durable rows are not current graph authority.
...
`fkst.test` 不存在于 `run`、`supervise`、`--self-test` 或 conformance production Lua state。`mock_command` 劫持 test mode 中的 `exec_sync`、codex SDK 与 git SDK 外部命令调用；未 mock 的外部命令 fail closed。`mock_observe(snapshot)` makes `fkst.observe()` use the provided deterministic raw snapshot for that test, including fresh Lua states spawned by `run_department`, `fire_raiser`, or `run_graph`; `fkst.observe(opts)` still applies `since`, `limit`, `include`, and DLQ `page` selection to the mock. Unmocked test-mode `fkst.observe()` fails closed.
...
`M.spec.published_seam` declares consumed queues that this package exposes as public sibling entry points. Graph scan requires each entry to be owned and consumed by the declaring Department, and requires every sibling Department or Raiser `produces` target to be either an own-package queue or one of these published entries. `M.spec.retry` may be omitted, set to `false`, or set to a table; `retry=true` is rejected. `M.spec.stall_window` is the reliable-delivery lease and renewal window, not a framework-child no-output kill deadline. There is no per-Department codex timeout or sandbox knob in `M.spec`; each `spawn_codex_sync(opts)` or `spawn_codex(opts)` call selects its timeout and sandbox mode through `opts.timeout` and `opts.sandbox`. The timeout defaults to 3600 seconds, and an omitted sandbox selects the bypass default. The global codex concurrency limit comes from `FKST_CODEX_PERMIT_SLOTS` / `codex_permit_slots`, not a per-Department timeout.
