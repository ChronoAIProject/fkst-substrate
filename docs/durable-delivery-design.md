...
`fkst-framework observe --durable-root <path> [--json] [--limit <n>]` is the read-only CQRS snapshot projection for the durable delivery store. When a live `supervise` process owns the `redb` handle, `supervise` serves a short host-local Unix socket derived from the durable root and `observe` reads that owner-process projection instead of opening `<DURABLE>/delivery.redb`. The owner retries socket accept failures with a bounded delay so one transport error does not terminate the projection. If no live socket exists, `observe` attempts offline inspection by opening `<DURABLE>/delivery.redb`. Both paths use the same storage projection in one read transaction: exact queue depth / pending / in-flight / retrying / oldest pending age comes from a separate live-record aggregate scan, while the live list follows `delivery_by_observe_order` `(queue, dept, not_before_ms, delivery_id)` and the DLQ follows `dead_by_time` `(dead_at_ms, delivery_id)`. Each requested listing reads at most `limit + 1` records, with the extra record determining its exact truncation flag. If the offline open reports that the database remains exclusively owned, `observe` fails with `observe-live-owner-unavailable` and directs the operator to restart the database-owning `supervise` process to restore the live endpoint, or stop that process before offline inspection. `--json` is the stable scripting interface; human output is only for local inspection.

`fkst.observe({ include = ... })` pushes its requested sections into that storage projection before selecting the live-owner socket or offline path. `queues` enables the exact aggregate scan, `events` enables the bounded live listing, `errors` enables the bounded DLQ listing, and `entities` enables both listings because it exposes their exact truncation metadata. Omitting `include` requests all three storage sections and preserves the existing wire shape. Projection changes read work only; returned SDK sections still follow the established `include` filtering contract.

Queue `subscriber_status` is `"current"`, `"absent"`, or `"unknown"`. The live owner-process projection reports `"current"` when the loaded graph has a current consumer for the queue and `"absent"` when it has none. The offline database fallback reports `"unknown"` because the durable rows alone do not authorize current topology conclusions. The compatibility field `has_current_subscriber` is still emitted in the live path where it already existed, but consumers should use `subscriber_status` for non-ambiguous state.
...
`redb` 的事务模型提供 MVCC read snapshots inside an opened database handle, but `redb 2.6.3` opens the database file with an exclusive process lock. Therefore live observation must not open the same database file from a second process while `supervise` holds the handle. The live path is the owner-process transient socket projection; the offline fallback may open `<DURABLE>/delivery.redb` only when no live socket is available. The socket is transient observability surface and does not become a durable fact source.

## 实现落点

1. redb API uses `delivery_by_id`, `delivery_by_observe_order`, `ready_by_dept_due`, `leased_by_dept_until`, `dead_by_id`, `dead_by_time`, and `meta`; all state transitions maintain their primary rows and secondary indexes in the same write transaction.
