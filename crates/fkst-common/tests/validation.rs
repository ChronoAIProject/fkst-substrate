...
#[test]
fn mixed_retry_consumers_on_fanout_queue_rejected() {
    let tmp = tempdir().unwrap();
    let lua = touch(tmp.path(), "d.lua");
    let mut cfg = cfg_minimal(&lua);
    cfg.queue.get_mut("tick").unwrap().fanout = true;
    ...
    assert!(e.to_string().contains("mixed retry consumers"), "{}", e);
    assert!(e.to_string().contains("retry enabled"), "{}", e);
    assert!(e.to_string().contains("retry disabled"), "{}", e);
}
