...
#[cfg(unix)]
#[test]
fn spawn_codex_sync_reports_whether_a_result_was_produced_or_adopted() {
    // An adopted five-second read and a real hour-long run otherwise report the
    // same thing. #3919 took a measurement of ELAPSED_MS to identify precisely
    // because the result carried no provenance.
    let tmp = tempfile::tempdir().unwrap();
    let bin_dir = tmp.path().join("bin");
    let worktree = tmp.path().join("wt");
    let capture_dir = tmp.path().join("capture");
...
"#,
    ...
    first_opts
        .set("worktree", worktree.to_string_lossy().into_owned())
        .unwrap();
    first_opts.set("dedup_key", "provenance-dedup").unwrap();
    let first: Table = spawn.call(first_opts).unwrap();
    assert_eq!(first.get::<i64>("exit_code").unwrap(), 0);
    assert_eq!(first.get::<String>("provenance").unwrap(), "produced");

    let second_opts = lua_opts(&lua, "same-work");
    second_opts
        .set("worktree", worktree.to_string_lossy().into_owned())
        .unwrap();
    second_opts.set("dedup_key", "provenance-dedup").unwrap();
    let second: Table = spawn.call(second_opts).unwrap();
    assert_eq!(second.get::<String>("stdout").unwrap(), "result-1");
    assert_eq!(
        second.get::<String>("provenance").unwrap(),
        "adopted",
        "a dispatch that read another run's record must say so"
    );
    let run_id = second
        .get::<String>("run_id")
        .expect("an adopted result must name the run that produced it");
...
#[cfg(unix)]
#[test]
fn spawn_codex_sync_probes_codex_effect_log_before_repeating_effect() {
    let tmp = tempfile::tempdir().unwrap();
    let bin_dir = tmp.path().join("bin");
    let worktree = tmp.path().join("wt");
    let capture_dir = tmp.path().join("capture");
...
"#,
    ...
    let spawn: mlua::Function = lua.globals().get("spawn_codex_sync").unwrap();
    let worktree_arg = worktree.to_string_lossy().into_owned();
    let opts = lua_opts(&lua, "external-effect-probe");
    opts.set("worktree", worktree_arg).unwrap();
    opts.set("dedup_key", "recover-external-effect").unwrap();
    let first: Table = spawn.call(opts.clone()).unwrap();
    assert_eq!(first.get::<String>("stdout").unwrap(), "external-effect-1");
    assert_eq!(
        std::fs::read_to_string(capture_dir.join("spawns")).unwrap(),
        "1"
    );
    ...
    let adoption_path = adoption_work_dir(tmp.path());
    for file in ["effect.json", "result.json", "stdout.txt", "stderr.txt"] {
        std::fs::remove_file(adoption_path.join(file)).unwrap();
    }
    let status_path = adoption_path.join("status.json");
    let mut status: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&status_path).unwrap()).unwrap();
    ...
    let recovered: Table = spawn.call(opts.clone()).unwrap();
    assert_eq!(
        recovered.get::<String>("stdout").unwrap(),
        "external-effect-1"
    );
    assert_eq!(
        std::fs::read_to_string(capture_dir.join("spawns")).unwrap(),
