...
fn write_host_package_manifest_at(root: &std::path::Path, name: &str, lib_deps: &[&str]) {
    let deps = lib_deps
        .iter()
...
#[test]
fn host_external_publishable_gate_does_not_filter_platform_package_same_source_lib_deps() {
    let root = tempfile::Builder::new()
        .prefix("host-publishable-gate")
        .tempdir()
        .unwrap();
    let host = root.path().join("host");
    let platform = host.join(".fkst/cache/fkst-packages");
    let platform_package = platform.join("packages/idle-detector");
    let host_package = host.join("packages/host-app");
    fs::create_dir_all(platform.join("libraries/contract/public")).unwrap();
    fs::create_dir_all(platform.join("libraries/workflow/public")).unwrap();
    fs::create_dir_all(platform.join("libraries/testkit/public")).unwrap();
    fs::create_dir_all(platform_package.join("departments/idle")).unwrap();
    fs::create_dir_all(platform_package.join("raisers")).unwrap();
    fs::create_dir_all(host_package.join("departments/probe")).unwrap();
...
"#,
    ...
    .unwrap();
    write_host_package_manifest_at(&host_package, "host-app", &[]);
    fs::write(
        host_package.join("departments/probe/main.lua"),
        r#"
local M = {}
M.spec = { consumes = { "idle-detector.idle" }, stall_window = "30s" }
function pipeline(_) end
return M
"#,
    )
    ...
    fs::write(
        platform.join("fkst.workspace.toml"),
        r#"
[workspace]
...
"#,
    ...
    .unwrap();
    write_host_package_manifest_at(&platform_package, "idle-detector", &["workflow"]);
    fs::write(
        platform_package.join("departments/idle/main.lua"),
        r#"
local workflow = require("workflow.core")
local M = {}
M.spec = { consumes = { "tick" }, produces = { "idle" }, stall_window = "30s" }
function pipeline(_) return workflow.value end
return M
"#,
    )
    .unwrap();
    fs::write(
        platform_package.join("raisers/idle.lua"),
        r#"return { type = "cron", interval = "10s", produces = "tick" }"#,
    )
    .unwrap();
    write_library_manifest_at(&platform.join("libraries/contract"), "contract", &[], true);
    fs::write(
        platform.join("libraries/contract/public/api.lua"),
        "return {}\n",
    )
    .unwrap();
    write_library_manifest_at(&platform.join("libraries/workflow"), "workflow", &[], false);
    fs::write(
        platform.join("libraries/workflow/public/core.lua"),
        "return { value = 'workflow' }\n",
    )
    .unwrap();
    write_library_manifest_at(&platform.join("libraries/testkit"), "testkit", &[], false);
    fs::write(
        platform.join("libraries/testkit/public/api.lua"),
        "return {}\n",
    )
    .unwrap();
    ...
    let config_email = Command::new("git")
        .arg("-C")
        .arg(&platform)
        .args(["config", "user.email", "fkst-test@example.invalid"])
        .output()
        .unwrap();
    assert_exit(&config_email, 0);
    let config_name = Command::new("git")
        .arg("-C")
        .arg(&platform)
        .args(["config", "user.name", "fkst test"])
        .output()
        .unwrap();
    assert_exit(&config_name, 0);
    ...
    let commit = Command::new("git")
        .arg("-C")
        .arg(&platform)
        .args(["commit", "-m", "Add platform package and libraries"])
        .output()
        .unwrap();
    assert_exit(&commit, 0);
    let rev_output = Command::new("git")
        ...
        .output()
        .unwrap();
    assert_exit(&rev_output, 0);
    let rev = String::from_utf8_lossy(&rev_output.stdout)
        .trim()
        .to_string();
    let workspace_path = host.join("fkst.workspace.toml");
    let workspace = fs::read_to_string(&workspace_path)
        .unwrap()
        .replace("__PLATFORM_GIT__", &platform.display().to_string())
        .replace("__PLATFORM_REV__", &rev);
    fs::write(&workspace_path, workspace).unwrap();

    let cache = root.path().join("cache");
    let lock_output = Command::new(framework_bin())
        .arg("deps")
        .arg("lock")
        .arg("--project-root")
        .arg(&host)
    ...
    let pass_args = [
        ...
        path_arg(&host),
        std::ffi::OsStr::new("--package-root"),
        path_arg(&host_package),
        std::ffi::OsStr::new("--package-root"),
        path_arg(&platform_package),
    ];
    let pass_output = run_conformance_with_env(
        &pass_args,
        &host,
        &[
            (RUNTIME_ROOT_ENV, root.path()),
            ("FKST_CACHE_ROOT", cache.as_path()),
        ],
    );

    assert_exit(&pass_output, 0);
    let pass_log = combined_log(&pass_output);
    assert!(pass_log.contains("PASS graph-scan"), "{pass_log}");
    assert!(
        !pass_log.contains("declares unknown library `workflow`"),
        "{pass_log}"
    );

    write_host_package_manifest_at(&host_package, "host-app", &["workflow"]);
    fs::write(
        host_package.join("departments/probe/main.lua"),
        r#"
local workflow = require("workflow.core")
...
"#,
    ...
    .unwrap();
    let fail_output = run_conformance_with_env(
        &pass_args,
        &host,
        &[
            (RUNTIME_ROOT_ENV, root.path()),
    ...
    assert_exit(&fail_output, 2);
    let fail_log = combined_log(&fail_output);
    assert!(
        fail_log.contains("external source `fkst-platform` does not allow library `workflow`"),
        "{fail_log}"
    );
}

#[test]
fn host_external_source_contract_only_still_resolves_platform_same_source_lib_dep() {
    let root = tempfile::Builder::new()
        .prefix("host-faithful-publishable-gate")
        .tempdir()
        .unwrap();
    let host = root.path().join("host");
    let platform = host.join(".fkst/run/fkst-packages-conformance");
    let platform_package = platform.join("packages/idle-detector");
    let host_package = host.join(".fkst/local-packages/site-board");
    let host_std = host.join(".fkst/std");
    fs::create_dir_all(platform.join("libraries/contract/public")).unwrap();
    fs::create_dir_all(platform.join("libraries/workflow/public")).unwrap();
    fs::create_dir_all(platform.join("libraries/testkit/public")).unwrap();
    fs::create_dir_all(platform_package.join("departments/idle")).unwrap();
...
"#,
    ...
    .unwrap();
    fs::write(
        host.join(".fkst/conformance/package-roots"),
        ".fkst/local-packages/site-board\nfkst-packages:packages/idle-detector\n",
    )
    .unwrap();
    write_host_package_manifest_at(&host_package, "site-board", &["std", "contract"]);
    fs::write(
        host_package.join("departments/board_scan/main.lua"),
        r#"
local contract = require("contract.core")
local text = require("std.text")
local M = {}
M.spec = { consumes = { "board_poll_tick", "idle-detector.idle" }, stall_window = "30s" }
function pipeline(_) return contract.value .. text.trim(" ok ") end
return M
"#,
    )
    .unwrap();
...
local contract = require("contract.core")
local M = {}
M.spec = { consumes = { "tick" }, produces = { "idle" }, stall_window = workflow.window }
function pipeline(_) return workflow.value .. contract.value end
return M
"#,
    )
    .unwrap();
    ...
    write_library_manifest_at(&platform.join("libraries/contract"), "contract", &[], true);
    fs::write(
        platform.join("libraries/contract/public/core.lua"),
        "return { value = 'contract' }\n",
    )
    .unwrap();
    write_library_manifest_at(
        &platform.join("libraries/workflow"),
        "workflow",
        &["contract"],
        false,
    );
    fs::write(
        platform.join("libraries/workflow/public/core.lua"),
        "return { value = 'workflow', window = '30s' }\n",
    )
    .unwrap();
    write_library_manifest_at(&platform.join("libraries/testkit"), "testkit", &[], false);
    fs::write(
    ...
    assert_exit(&lock_output, 0);
    let workspace_path = host.join("fkst.workspace.toml");
    let workspace = fs::read_to_string(&workspace_path).unwrap().replace(
        "  \".fkst/std\",\n]",
        "  \".fkst/std\",\n  \".fkst/run/fkst-packages-conformance/packages/idle-detector\",\n]",
    );
    fs::write(&workspace_path, workspace).unwrap();
    ...
    assert!(pass_log.contains("PASS graph-scan"), "{pass_log}");
    assert!(
        !pass_log.contains("external source `fkst-platform` does not allow library `workflow`"),
        "{pass_log}"
    );
    assert!(
        !pass_log.contains("declares unknown library `workflow`"),
...
local M = {}
M.spec = { consumes = { "board_poll_tick", "idle-detector.idle" }, stall_window = "30s" }
function pipeline(_) return contract.value .. text.trim(" ok ") .. workflow.value end
return M
"#,
    )
    .unwrap();
