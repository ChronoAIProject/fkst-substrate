...
fn boundary_resources_lists_mediated_resource_registry() {
    ...
    assert!(out.contains("id=argv.process"), "{out}");
    assert!(out.contains("adapters=exec_argv"), "{out}");
    assert!(out.contains("id=host.environment"), "{out}");
    assert!(out.contains("adapters=env_read"), "{out}");
    assert!(
